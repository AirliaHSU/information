...
5. 執行 `npm test`。兩版與輪播圓點會自動更新，不必修改 HTML。
...
所有作品與聯絡連結都已存在產生的 HTML；停用 JavaScript 時，動畫作品改為垂直排列。
啟用 JavaScript 後使用 10 秒輪播；滑鼠停留、鍵盤焦點進入或分頁隱藏時暫停。
支援手動暫停與系統減少動態效果偏好。影片仍使用 YouTube 嵌入。

`npm test` 驗證兩版共用作品、求職版無接案 CTA、方案預填、metadata、資源路徑及資料更新同步。
發布前另檢查桌面與手機寬度的排版，以及實際 YouTube 播放和本機郵件程式開啟行為。
